...
export async function getStripeData() {
  ...
  const [balance, charges90d, subscriptions, customers, invoicesMonth, invoicesLastMonth, payouts, disputes] =
    await Promise.all([
  ...
  for (let i = 89; i >= 0; i--) {
    dailyRevenue[new Date(Date.now() - i * 86400000).toISOString().slice(0, 10)] = 0
  }
  ...
  return {
    ...
    revenueTimeSeries: Object.entries(dailyRevenue).map(([date, amount]) => ({ date, amount })),
    recentCharges: charges90d.data.slice(0, 30).map(c => ({
      id: c.id, amount: c.amount / 100, currency: c.currency, status: c.status,
      refunded: c.refunded, customer: c.customer, description: c.description,
    ...
    openDisputeCount: disputes.data.filter(d => d.status !== 'won' && d.status !== 'lost').length,
    disputes: disputes.data
      .filter(d => d.status !== 'won' && d.status !== 'lost')
      .slice(0, 5)
      .map(d => ({
        id: d.id, amount: d.amount / 100, reason: d.reason,
        status: d.status, created: new Date(d.created * 1000).toISOString(),
      })),
  }
}
